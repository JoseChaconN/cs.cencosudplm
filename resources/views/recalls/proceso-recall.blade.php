	...
	<div class="col-lg-12">
		<div class="card shadow ">
			<div class="card-header py-3">
				<h6 class="m-0 font-weight-bold text-primary">Recall en proceso N° {{$recall->id}} del proveedor {{$recall->nombre_proveedor}}</h6>
			</div>
			<div class="card-body border-left-primary">
				<div class="row">
							...
							<table class="table table-bordered" width="100%" cellspacing="0">
								...
								<tbody>
									<tr>
										...
										<td><a class="btn btn-primary" href="{{route('procesoDetalleRecall',$recall->id)}}">Ver/Editar</a></td>
									</tr>
								</tbody>
							</table>
						</div>
					...
					<div class="col-md-12">
						@if (($recall->id_responsable == Auth::user()->id || Auth::user()->hasRole('administrador')) && $recall->status == 'PROCESO')
							<button class="btn btn-danger" type="button" data-toggle="modal" data-target="#cerrarRecallModal">Cerrar recall</button>
							@include('recalls.components.cerrar-recall-modal')
							
						@endif
						@if (($recall->id_responsable == Auth::user()->id || Auth::user()->hasRole('administrador')) && $recall->status == 'CERRADO')
							@if (!empty($recall->getMedia('documento-cierre-recall')->last()))
								<a class="btn btn-success" href="{{ $recall->getMedia('documento-cierre-recall')->last()->getUrl() }}" download="">Descargar documento de cierre</a>
							@endif
							<button class="btn btn-primary" type="button" data-toggle="modal" data-target="#abrirRecallModal">Abrir recall</button>
							@include('recalls.components.abrir-recall-modal')
						@endif
						<a class="btn btn-primary" href="{{route('respuestaRecall',$recall->id)}}">Responder como local</a>
